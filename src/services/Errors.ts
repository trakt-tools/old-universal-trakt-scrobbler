...
class _Errors {
  ...
  startListeners(): void {
    EventDispatcher.subscribe(Events.SCROBBLE_ERROR, (data: ErrorEventData) => this.onItemError(data, 'scrobble'));
    EventDispatcher.subscribe(Events.SEARCH_ERROR, (data: ErrorEventData) => this.onItemError(data, 'find'));
  }

  async onItemError(data: ErrorEventData, type: string): Promise<void> {
   if (data.error) {
